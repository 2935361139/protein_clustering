...
class GenerateTFRecord(object):
    ...
    def _serialize_example(self, prot_id, sequence, ca_dist_matrix, cb_dist_matrix):
        FIXED_LENGTH = 160
        labels = self._dtype_feature()  # 确保此函数已正确定义

        # 校验输入尺寸
        assert len(sequence) == FIXED_LENGTH, f"序列长度错误: {len(sequence)}"
        assert ca_dist_matrix.shape == (FIXED_LENGTH, FIXED_LENGTH), f"CA矩阵尺寸错误: {ca_dist_matrix.shape}"
        assert cb_dist_matrix.shape == (FIXED_LENGTH, FIXED_LENGTH), f"CB矩阵尺寸错误: {cb_dist_matrix.shape}"

        d_feature = {
            'prot_id': self._bytes_feature(prot_id.encode()),
            'seq_1hot': self._float_feature(seq2onehot(sequence).reshape(-1)),
            'L': self._int_feature(FIXED_LENGTH),
            'ca_dist_matrix': self._float_feature(ca_dist_matrix.reshape(-1)),
            'cb_dist_matrix': self._float_feature(cb_dist_matrix.reshape(-1)),
        }
...
if __name__ == "__main__":
    ...
    parser.add_argument('-prot_list', type=str, default='D:/bjfu/GNN-transformer/protein-predictor/output/4test.txt',
                        help="Input file (*.txt) with a set of protein IDs with distMAps in npz_dir.")
    parser.add_argument('-npz_dir', type=str, default='D:/bjfu/GNN-transformer/protein-predictor/data/output-npz/',
    ...
    parser.add_argument('-num_shards', type=int, default=4, help="Number of tfrecord files per protein set.")
    parser.add_argument('-tfr_prefix', type=str, default='D:/bjfu/GNN-transformer/protein-predictor/data/TFRecords/PDB_GO_train',
